...
3. **Create an API** (Auth0 dashboard → Applications → APIs → Create API):
   ...
   - Open the API → **Settings tab** → **Access Settings** panel → check **Allow Offline Access**.
     Defaults to off for a newly created API. Without it, Auth0 silently drops the `offline_access`
     scope and never issues a refresh token, so login succeeds but `carqna-copilot-ui` fails later
     once the short-lived access token expires: `Error [AccessTokenError]: The access token has
     expired and a refresh token was not provided` (`code: 'missing_refresh_token'`). If you hit this
     after already logging in once, toggling the setting isn't retroactive — log out
     (`http://localhost:3000/auth/logout`) and back in to get a token set that includes a refresh
     token.
